...
~~現在の Azure AD には、ユーザーの最終サインイン日時を保持するプロパティがないため、ユーザーがいつ最後にサインインしたかという時刻を取得することはできません。Azure AD の開発チームは、プロパティの作成を検討しておりますが、明確な機能追加の時期は発表されていません。~~

> [SignInActivity](https://learn.microsoft.com/en-us/graph/api/resources/signinactivity?view=graph-rest-1.0) プロパティが一般公開され、最終サインイン日時が取得可能となりました
> [master ブランチ](https://github.com/jpazureid/get-last-signin-reports) にて詳細な手順をご紹介しておりますので、よろしければご確認ください。
...
- サインインのログから一覧を取得しているため、アプリへの厳密な ”最終アクセス日時” とは異なります。
